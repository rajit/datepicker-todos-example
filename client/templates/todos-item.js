...
Template.todosItem.events({
  ...
  'focus input[type=text].task-name': function(event) {
    Session.set(EDITING_KEY, this._id);
  },
  
  'blur input[type=text].task-name': function(event) {
    if (Session.equals(EDITING_KEY, this._id))
      Session.set(EDITING_KEY, null);
  },
  
  'keydown input[type=text].task-name': function(event) {
    // ESC or ENTER
  ...
  // every 300ms)
  'keyup input[type=text].task-name': _.throttle(function(event) {
